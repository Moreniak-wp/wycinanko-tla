// CAŁY POPRZEDNI KOD POZOSTAJE BEZ ZMIAN...
...
toleranceSlider.addEventListener('input', function() { toleranceValue.textContent = this.value; });
marginSlider.addEventListener('input', function() { marginValue.textContent = this.value; });
imageInput.addEventListener('change', function() {
  ...
  if (filesToAdd.length < newFiles.length) { alert('Niektóre z wybranych plików już znajdują się na liście i zostały pominięte.'); }
  if (filesToAdd.length === 0) { this.value = null; return; }
...
function updateFileList() {
  ...
  loadedImages.forEach((item, index) => {
    ...
    fileItem.innerHTML = `<span onclick="displayImage(${index})" style="cursor: pointer; flex: 1;">${item.name}</span><button onclick="removeImage(event, ${index})">×</button>`;
...
function displayImage(index) {
  ...
  if (item.processed) {
    processedCanvas.width = item.processed.width; processedCanvas.height = item.processed.height;
...
function processImage(imageData, width, height, tolerance, doRemoveColumns, doRemoveRows) {
  const columnsToKeep = [];
  if (doRemoveColumns) { for (let x = 0; x < width; x++) { if (!isColumnUniform(imageData, x, width, height, tolerance)) columnsToKeep.push(x); }
  } else { for (let x = 0; x < width; x++) columnsToKeep.push(x); }
  const rowsToKeep = [];
  if (doRemoveRows) { for (let y = 0; y < height; y++) { if (!isRowUniform(imageData, y, width, height, tolerance)) rowsToKeep.push(y); }
  } else { for (let y = 0; y < height; y++) rowsToKeep.push(y); }
  if (columnsToKeep.length === 0 || rowsToKeep.length === 0) return null;
  const newWidth = columnsToKeep.length; const newHeight = rowsToKeep.length;
  ...
  for (let newY = 0; newY < newHeight; newY++) {
    ...
    for (let newX = 0; newX < newWidth; newX++) {
      ...
      for (let i = 0; i < 4; i++) newImageData.data[newIndex + i] = imageData.data[oldIndex + i];
    }
  }
  return newImageData;
}
const processSingle = (processingFunction) => {
  if (loadedImages.length === 0) { alert('Czy tobie się wąski sufit na łeb nie spadł? Plik daj najpierw dzbanie jeden'); return; }
  const currentItem = loadedImages[currentImageIndex];
  const processedImageData = processingFunction(currentItem.image);
  if (processedImageData) { currentItem.processed = processedImageData; displayImage(currentImageIndex); }
  else { alert('Chyba cię pojebało, usunąłeś wszystko co się dało'); }
};
...
function getImageForScaling(image) {
  ...
  if (!doRemoveColumns && !doRemoveRows) { alert('knyfla przynajmniej jednego kliknij no'); return null; }
  const tempCtx = document.createElement('canvas').getContext('2d');
  tempCtx.canvas.width = image.width; tempCtx.canvas.height = image.height;
...
document.getElementById('downloadBtn').addEventListener('click', function() {
  const currentItem = loadedImages[currentImageIndex];
  if (!currentItem || !currentItem.processed) { alert('Najpierw przetwórz obraz!'); return; }
  const tempCanvas = document.createElement('canvas');
  tempCanvas.width = currentItem.processed.width; tempCanvas.height = currentItem.processed.height;
...
document.getElementById('downloadAllBtn').addEventListener('click', async function() {
  ...
  if (processedItems.length === 0) { alert('Najpierw przetwórz przynajmniej jeden obraz!'); return; }
  for (const item of processedItems) {
    const tempCanvas = document.createElement('canvas');
    tempCanvas.width = item.processed.width; tempCanvas.height = item.processed.height;
...
canvas.addEventListener('click', function(e) {
  ...
  try {
    ...
    if (box) { box.style.background = hex; box.textContent = `${hex} rgba(${px[0]},${px[1]},${px[2]},${(px[3]/255).toFixed(2)})`; }
  } catch (err) { /* ignore */ }
});
...
document.getElementById('doomBtn').addEventListener('click', function() {
  const overlay = document.getElementById('doomOverlay');
  const gif = document.getElementById('doomGif');

  gif.src = 'https://gifdb.com/images/thumbnail/deltarune-explosion-white-background-f3frnjqdtz6ykv0x.gif'; 
